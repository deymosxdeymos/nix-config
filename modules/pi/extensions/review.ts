 ...
 *
 * Note: in git repos, PR review requires a clean working tree (no uncommitted changes to
 * tracked files). jj repos are detected automatically and use jj-native commands (jj diff,
 * bookmarks, revsets); jj PR review fetches the PR head and reviews it via revset without
 * mutating the working copy, so it also works in jj workspaces.
 */
...
// Which version control system the active review is using.
// jj repos (including secondary workspaces) need jj-native commands because git
// commands either fail or operate on a different working copy.
type Vcs = "git" | "jj";
let activeVcs: Vcs = "git";

/**
 * Detect whether the cwd is a jj or git repo. Prefer jj when both are present
 * (colocated repo) since the user is driving with jj.
 */
async function detectVcs(pi: ExtensionAPI): Promise<Vcs | null> {
  const jj = await pi.exec("jj", ["root"]);
...
type ReviewTarget =
  ...
  | {
      ...
      baseBranch: string;
      title: string;
      // jj-only: the PR head commit id, used to review via revset without checkout
      headRef?: string;
    }
...
// jj-native prompt variants (used when the repo is detected as jj)
const JJ_UNCOMMITTED_PROMPT =
  "Review the current working-copy changes. Run `jj st` and `jj diff` to inspect the changes in the working-copy commit (@), including new files. If the working copy is empty, inspect its parent with `jj diff -r @-` instead. Provide prioritized, actionable findings.";

const JJ_BASE_BRANCH_PROMPT =
  "Review the code changes against the base '{branch}'. Run `jj diff --from 'heads(::@ & ::\"{branch}\")' --to @` to inspect the changes that would be merged into {branch} (the `--from` revset resolves the merge base of @ and {branch}). Provide prioritized, actionable findings.";

const JJ_COMMIT_PROMPT_WITH_TITLE =
  'Review the code changes introduced by revision {sha} ("{title}"). Run `jj diff -r {sha}` to inspect them. Provide prioritized, actionable findings.';

const JJ_COMMIT_PROMPT =
  "Review the code changes introduced by revision {sha}. Run `jj diff -r {sha}` to inspect them. Provide prioritized, actionable findings.";

const JJ_PULL_REQUEST_PROMPT =
  "Review pull request #{prNumber} (\"{title}\") against the base branch '{baseBranch}'. The PR head commit is {headRef}. Run `jj diff --from 'heads(::{headRef} & ::\"{baseBranch}@origin\")' --to {headRef}` to inspect the changes that would be merged. If revision {headRef} is not available locally, run `gh pr diff {prNumber}` to inspect the diff instead. Provide prioritized, actionable findings.";
...
async function getLocalBranches(pi: ExtensionAPI): Promise<string[]> {
  if (activeVcs === "jj") {
    const { stdout, code } = await pi.exec("jj", ["bookmark", "list", "-T", 'name ++ "\\n"']);
    if (code !== 0) return [];
    return stdout
      .trim()
      .split("\n")
  ...
  if (activeVcs === "jj") {
    ...
    return stdout
      ...
      .map((line) => {
        ...
        return { sha: sha.trim(), title: rest.join("\t").trim() };
      });
  }
...
async function hasUncommittedChanges(pi: ExtensionAPI): Promise<boolean> {
  if (activeVcs === "jj") {
    // The jj working copy is always a commit; "uncommitted" means @ is non-empty.
    const { stdout, code } = await pi.exec("jj", ["log", "--no-graph", "-r", "@", "-T", "empty"]);
...
async function hasPendingChanges(pi: ExtensionAPI): Promise<boolean> {
  // jj never loses working-copy changes when moving around, and PR review does not
  // mutate the working copy, so there is nothing to block on.
  if (activeVcs === "jj") return false;
...
async function getPrInfo(
  ...
  const { stdout, code } = await pi.exec("gh", [
    ...
    String(prNumber),
    "--json",
    "baseRefName,title,headRefName,headRefOid",
  ]);
...
async function getCurrentBranch(pi: ExtensionAPI): Promise<string | null> {
  // jj has no single "current branch" (the working copy is often anonymous), so there
  // is nothing meaningful to exclude from the base-branch selector.
  if (activeVcs === "jj") return null;
 ...
 */
function buildJjReviewPrompt(target: ReviewTarget): string {
  switch (target.type) {
    case "uncommitted":
      return JJ_UNCOMMITTED_PROMPT;

    case "baseBranch":
      return JJ_BASE_BRANCH_PROMPT.replace(/{branch}/g, target.branch);

    case "commit":
      return target.title
        ? JJ_COMMIT_PROMPT_WITH_TITLE.replace(/{sha}/g, target.sha).replace("{title}", target.title)
        : JJ_COMMIT_PROMPT.replace(/{sha}/g, target.sha);

    case "pullRequest": {
      const headRef = target.headRef ?? `${target.baseBranch}@origin..`;
      return JJ_PULL_REQUEST_PROMPT.replace(/{prNumber}/g, String(target.prNumber))
        .replace(/{title}/g, target.title)
        .replace(/{baseBranch}/g, target.baseBranch)
        .replace(/{headRef}/g, headRef);
    }
...
async function buildReviewPrompt(pi: ExtensionAPI, target: ReviewTarget): Promise<string> {
  if (activeVcs === "jj") {
    return buildJjReviewPrompt(target);
  }
...
export default function reviewExtension(pi: ExtensionAPI) {
  ...
  async function resolvePullRequestTarget(
    ...
    // jj: don't check out (that would mutate the working copy and breaks in workspaces).
    // Fetch the PR head and review it via revset against the base.
    if (activeVcs === "jj") {
      ctx.ui.notify(`Fetching latest changes for PR #${prNumber}...`, "info");
      const fetch = await pi.exec("jj", ["git", "fetch"]);
      if (fetch.code !== 0) {
        ctx.ui.notify(
          `Failed to fetch PR refs: ${fetch.stderr || fetch.stdout || "jj git fetch failed"}`,
          "error",
        );
        return null;
      }

      ctx.ui.notify(`Reviewing PR #${prNumber} (${prInfo.headBranch})`, "info");
  ...
  async function getSmartDefault(): Promise<"uncommitted" | "baseBranch" | "commit"> {
    ...
    // jj has no "current branch"; default to reviewing the stack against the base.
    if (activeVcs === "jj") {
      return "baseBranch";
    }
  ...
  pi.registerCommand("review", {
    ...
    handler: async (args, ctx) => {
      ...
      // Detect the version control system (prefers jj when colocated with git)
      const vcs = await detectVcs(pi);
      if (!vcs) {
        ctx.ui.notify("Not a git or jj repository", "error");
        return;
      }
      activeVcs = vcs;
