...
bookmarks. Use `stack` only for stack intent, inspection, sync, merge, and undo.
...
Stack intent is persisted in `.git/stack/state.json` or
`.jj/repo/stack/state.json` as stack links (branch/bookmark, parent,
merge-base anchor, change number). Mutating workflows write the matching
`undo.json` so `stack undo --apply` can restore the previous state.
